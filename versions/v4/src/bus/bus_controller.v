...
写数据支持字节写使能功能,一个4位宽的写使能信号,能够对字节进行写入
...
module bus_controller(
    ...
    output reg  [ 3:0] gpio_wen ,  // GPIO写使能
...
localparam MEM_SIZE   = 32'h0000_0FFF;  // 4KB存储空间

localparam GPIO_BASE  = 32'h0000_2000;  // GPIO基地址   Slave2: GPIO
...
assign mem_addr = mem_sel ? (((cpu_addr - MEM_BASE) & 32'hFFFFFFFC) >> 2) : 0; //将地址映射为RAM地址(字地址),因为RAM是按字访问的，所以需要将地址右移2位,并且低两位清零
...
assign gpio_addr = gpio_sel ?(((cpu_addr - GPIO_BASE) & 32'hFFFFFFFC) >> 2) : 0; //将地址映射为RAM地址(字地址),因为RAM是按字访问的，所以需要将地址右移2位,并且低两位清零

// gpio_wen:写使能信号
always @(*) begin
    if (rst_n == 1'b0) begin
        gpio_wen = 4'b0000; //复位时写使能信号为0
    end
    else if(gpio_sel == 1'b1) begin
        if (cpu_ctrl == `MEM_ACCESS_TYPE_WRITE_WORD) begin
            gpio_wen = 4'b1111; //写使能信号为1111,表示写入32位数据
        end
        else if (cpu_ctrl == `MEM_ACCESS_TYPE_WRITE_HALF) begin
            case (offset) //根据地址的低两位判断是高16位还是低16位
                2'b00: gpio_wen = 4'b0011; //低16位写使能信号为0011,表示写入低16位数据
                2'b10: gpio_wen = 4'b1100; //高16位写使能信号为1100,表示写入高16位数据
                default: gpio_wen = 4'b0000; //其他情况不允许写入
            endcase
        end
        else if (cpu_ctrl == `MEM_ACCESS_TYPE_WRITE_BYTE) begin
            case (offset) //根据地址的低两位判断是第几字节
                2'b00: gpio_wen = 4'b0001; //第一个字节写使能信号为0001,表示写入第一个字节数据
                2'b01: gpio_wen = 4'b0010; //第二个字节写使能信号为0010,表示写入第二个字节数据
                2'b10: gpio_wen = 4'b0100; //第三个字节写使能信号为0100,表示写入第三个字节数据
                2'b11: gpio_wen = 4'b1000; //第四个字节写使能信号为1000,表示写入第四个字节数据
                default: gpio_wen = 4'b0000; //其他情况不允许写入
            endcase
        end
        else begin
            gpio_wen = 4'b0000; //其他情况不允许写入
        end
    end
    else begin
        gpio_wen = 4'b0000; //其他情况不允许写入
...
always @(*) begin
    if (rst_n == 1'b0) begin
        ...
        case (cpu_ctrl)
            ...
            `MEM_ACCESS_TYPE_WRITE_HALF: begin
                case (offset) //根据地址的低两位判断是高16位还是低16位
                    2'b00: gpio_wdata = (cpu_wdata & 32'h0000_FFFF)      ; //低16位写入数据
                    2'b10: gpio_wdata = (cpu_wdata & 32'h0000_FFFF) << 16; //高16位写入数据
                    default: gpio_wdata = 32'b0; //其他情况不允许写入
                endcase
            end
            `MEM_ACCESS_TYPE_WRITE_BYTE: begin
                case (offset) //根据地址的低两位判断是第几字节
                    2'b00: gpio_wdata = (cpu_wdata & 32'h0000_00FF)      ; //第一个字节写入数据
                    2'b01: gpio_wdata = (cpu_wdata & 32'h0000_00FF) <<  8; //第二个字节写入数据
                    2'b10: gpio_wdata = (cpu_wdata & 32'h0000_00FF) << 16; //第三个字节写入数据
                    2'b11: gpio_wdata = (cpu_wdata & 32'h0000_00FF) << 24;
                    default: gpio_wdata = 32'b0; //其他情况不允许写入
                endcase
            end
            default: begin
                gpio_wdata = 32'b0; //其他情况不允许写入
            end
        endcase
    end else begin
        gpio_wdata = 32'b0; //其他情况不允许写入
