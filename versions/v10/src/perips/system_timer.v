...
//     input  wire        sel,
//     // 寄存器读写接口
//     input  wire [2:0]  addr,         // 3位地址 000=CTRL, 001=LOAD_L, 010=LOAD_H, 011=VAL_L, 100=VAL_H
...
//                 3'b010: load_reg_h <= wdata;
//                 3'b011: begin  // 写VAL_L触发重载
...
//         if (ctrl_reg[0]) begin
//             // 低32位处理
//             {borrow_low, val_reg_l} <= val_reg_l - 1'b1;
//             // 高32位处理
//             if (borrow_low && val_reg_h > 0) begin
//                 val_reg_h <= val_reg_h - 1'b1;
//             end
//             // 计数器归零检测与重载
//             if ({val_reg_h, val_reg_l} == 64'h0) begin
//                 {val_reg_h, val_reg_l} <= {load_reg_h, load_reg_l};
