...
module bus_controller(
    ...
    // 系统定时器接口
    output wire        system_timer_sel, // 系统定时器片选
    output wire [31:0] system_timer_addr, // 系统定时器寄存器地址
    output reg         system_timer_wen, // 系统定时器写使能
    output reg  [31:0] system_timer_wdata, // 系统定时器写数据
    input  wire [31:0] system_timer_rdata, // 系统定时器读数据
    output wire        system_timer_irq // 系统定时器中断信号
);
...
// localparam MEM_BASE   = 32'h0040_1000;  // 存储器基地址  Slave1: DMEM
localparam MEM_SIZE   = 32'h0000_1FFF;  // 8KB存储空间
...
localparam SYSTEM_TIMER_BASE = 32'h1003_0200; // 系统定时器基地址 Slave4: TIMER
localparam SYSTEM_TIMER_SIZE = 32'h0000_0014; // 20字节空间
...
begin
    ...
    // 系统定时器地址范围检测
    else if(cpu_addr >= SYSTEM_TIMER_BASE && cpu_addr <= (SYSTEM_TIMER_BASE + SYSTEM_TIMER_SIZE)) begin
        system_timer_select = 1'b1;
    end
    // 非法地址访问
    else begin
        invalid_select = 1'b1;
...
always @(posedge clk or negedge rst_n) begin
    if (rst_n == 1'b0) begin
        ...
        system_timer_select_r <= system_timer_select;
...
// 系统定时器接口信号
//-----------------------------------------------------------------
assign system_timer_sel  = system_timer_select;
assign system_timer_addr = system_timer_sel ?(((cpu_addr - SYSTEM_TIMER_BASE) & 32'hFFFFFFFC) >> 2)  : 32'bz; //将地址映射为外设地址(字地址)

// system_timer_wen:写使能信号
always @(*) begin
    if (rst_n == 1'b0) begin
        system_timer_wen = 1'b0; //复位时写使能信号为0
    end
    else if(system_timer_sel == 1'b1) begin
        if (cpu_ctrl == `MEM_ACCESS_TYPE_WRITE_WORD || cpu_ctrl == `MEM_ACCESS_TYPE_WRITE_HALF || cpu_ctrl == `MEM_ACCESS_TYPE_WRITE_BYTE) begin
            system_timer_wen = 1'b1; 
        end
        else begin
            system_timer_wen = 1'b0; //其他情况不允许写入
        end
    end
    else begin
        system_timer_wen = 1'b0; //其他情况不允许写入
...
//system_timer_wdata:写数据
always @(*) begin
    if (rst_n == 1'b0) begin
        system_timer_wdata = 32'bz; //复位时写数据为0
    end
    else if (system_timer_sel == 1'b1) begin
        case (cpu_ctrl)
            `MEM_ACCESS_TYPE_WRITE_WORD: begin
                system_timer_wdata = cpu_wdata; //写入32位数据
            end
            `MEM_ACCESS_TYPE_WRITE_HALF: begin
                system_timer_wdata = (cpu_wdata & 32'h0000_FFFF);
            end
            `MEM_ACCESS_TYPE_WRITE_BYTE: begin
                system_timer_wdata = (cpu_wdata & 32'h0000_00FF);
            end
            default: begin
                system_timer_wdata = 32'bz; //其他情况不允许写入
            end
        endcase
    end else begin
        system_timer_wdata = 32'bz; //其他情况不允许写入
...
begin
    case (1'b1)
        ...
        system_timer_select_r: read_mux = system_timer_rdata;
