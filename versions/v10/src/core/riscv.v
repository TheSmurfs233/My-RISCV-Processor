...
module riscv (
    ...
    //下载接口
    input   wire                                 inst_meme_we_i,
    input   wire [`CPU_WIDTH - 1:0]              inst_meme_waddr_i,
    input   wire [`CPU_WIDTH - 1:0]              inst_meme_wdata_i
);
...
wire     mem_pipestall;     //访存存阶段流水线暂停信号
assign  pipeline_stall = alu_pipestall | mem_pipestall;
...
inst_mem u_inst_mem_0(
    ...
    .pc_addr_i                      ( curr_pc                       ),
    .inst_o                         ( inst                          ),
    .we_i                           ( inst_meme_we_i                ), // 指令存储器写操作
    .waddr_i                        ( inst_meme_waddr_i             ), // 指令存储器写地址
    .wdata_i                        ( inst_meme_wdata_i             )  // 指令存储器写数据
);
...
reg [1:0] mem_pipestall_cnt;
wire load_inst;  
assign load_inst = (ex_mem_mem_access_type == `MEM_ACCESS_TYPE_READ_BYTE || ex_mem_mem_access_type == `MEM_ACCESS_TYPE_READ_HALF || ex_mem_mem_access_type == `MEM_ACCESS_TYPE_READ_WORD) ? 1'b1 : 1'b0;
assign mem_pipestall = (mem_pipestall_cnt != 1 && load_inst == 1'b1) ? 1'b1 : 1'b0;

assign sys_bus_addr_o      = (ex_mem_mem_access_type != `MEM_ACCESS_TYPE_NONE) ? ex_mem_alu_res : `CPU_WIDTH'h0;    
assign mem_rdata           = sys_bus_rdata_i;
// assign sys_bus_wdata_o     = (ex_mem_mem_access_type == `MEM_ACCESS_TYPE_WRITE_BYTE || ex_mem_mem_access_type == `MEM_ACCESS_TYPE_WRITE_HALF || ex_mem_mem_access_type == `MEM_ACCESS_TYPE_WRITE_WORD) ? id_ex_reg2_rdata:`CPU_WIDTH'h0; //写数据
assign sys_bus_access_type = ex_mem_mem_access_type;


//mem_pipestall_cnt
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        mem_pipestall_cnt <= 2'd0;
    end
    else if (load_inst == 1'b0 || mem_pipestall_cnt == 2'd1) begin
        mem_pipestall_cnt <= 2'd0;
    end
    else if(load_inst == 1'b1) begin
        mem_pipestall_cnt <= mem_pipestall_cnt + 2'd1;
    end
    else begin
        mem_pipestall_cnt <= 2'd0;
    end
end
reg [31:0] sys_bus_wdata_o_r;
always @(posedge clk) begin
    sys_bus_wdata_o <= sys_bus_wdata_o_r;
end
...
always @(*) begin
    if (id_ex_mem_access_type == `MEM_ACCESS_TYPE_WRITE_BYTE || id_ex_mem_access_type == `MEM_ACCESS_TYPE_WRITE_HALF || id_ex_mem_access_type == `MEM_ACCESS_TYPE_WRITE_WORD) begin
        if (id_ex_forward_to_alu[1] == 1'b1) begin
            case (ex_mem_mem_access_type)
                `MEM_ACCESS_TYPE_READ_BYTE: begin
                    case (ex_mem_alu_res[1:0])
                        2'b00: sys_bus_wdata_o_r = ex_mem_mem_sign_ext ?{{24{mem_rdata[ 7]}},mem_rdata[ 7: 0]}:{{24{1'b0}},mem_rdata[ 7: 0]}; 
                        2'b01: sys_bus_wdata_o_r = ex_mem_mem_sign_ext ?{{24{mem_rdata[15]}},mem_rdata[15: 8]}:{{24{1'b0}},mem_rdata[15: 8]}; 
                        2'b10: sys_bus_wdata_o_r = ex_mem_mem_sign_ext ?{{24{mem_rdata[23]}},mem_rdata[23:16]}:{{24{1'b0}},mem_rdata[23:16]};  
                        2'b11: sys_bus_wdata_o_r = ex_mem_mem_sign_ext ?{{24{mem_rdata[31]}},mem_rdata[31:24]}:{{24{1'b0}},mem_rdata[31:24]};
                    endcase
                end
                `MEM_ACCESS_TYPE_READ_HALF: begin
                    case (ex_mem_alu_res[0])
                        1'b0: sys_bus_wdata_o_r = ex_mem_mem_sign_ext ?{{16{mem_rdata[15]}},mem_rdata[15: 0]}:{{16{1'b0}},mem_rdata[15: 0]}; 
                        1'b1: sys_bus_wdata_o_r = ex_mem_mem_sign_ext ?{{16{mem_rdata[31]}},mem_rdata[31:16]}:{{16{1'b0}},mem_rdata[31:16]};
                        endcase
                end
                `MEM_ACCESS_TYPE_READ_WORD: begin
                sys_bus_wdata_o_r = mem_rdata;
                end
                // default: 
            endcase
            ...
            sys_bus_wdata_o_r = id_ex_reg2_rdata;
        end
    end
    else  begin
        sys_bus_wdata_o_r = `CPU_WIDTH'hz; // 高阻态
    end
end
