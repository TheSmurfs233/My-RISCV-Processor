...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin // 当复位信号被激活时（rst_n为低电平）
        for (integer  i = 0; i <= `REG_DATA_DEPTH; i = i + 1) begin
            regs[i] <= `CPU_WIDTH'b0; // 所有寄存器归零
        end
    end 
    else begin 
        if (reg_wen && (reg_waddr != `REG_ADDR_WIDTH'b0)) // x0 read only
