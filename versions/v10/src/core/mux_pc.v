...
// wire [`CPU_WIDTH-1:0] pc_branch_offset = curr_pc_i + imm - `CPU_WIDTH'h8;
// wire [`CPU_WIDTH-1:0] pc_jalr_base = (forward_to_alu[0]) ? forward_reg1_rdata : reg1_rdata;
// wire [`CPU_WIDTH-1:0] pc_jalr_target = (pc_jalr_base + imm) & `CPU_WIDTH'hFFFFFFFC;
...
//     end
//     else if(branch && !zero && id_ex_control_hazard) begin  // bne分支指令,预测失败，zero为0时跳转
//         next_pc_o = pc_branch_offset;
//         pipe_flush = 1'b1;
//     end
//     else if(jump) begin     // jalr跳转指令
//         next_pc_o = (reg1_rdata > `RESET_PC_VALUE) ? pc_jalr_target : pc_branch_offset;
//         pipe_flush = 1'b1;
//     end
//     else begin
//         next_pc_o = curr_pc_i + `CPU_WIDTH'h4; // 默认情况，下一条指令地址为当前指令地址加4
