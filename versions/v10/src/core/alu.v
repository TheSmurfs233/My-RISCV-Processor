...
module alu(
    ...
    output                         alu_pipestall // 流水线暂停信号
);

parameter MUL_PIPE_STALL_CYCLES = 6; // 乘法器流水线暂停周期


reg   alu_pipestall_div; // 除法器流水线暂停信号
wire  alu_pipestall_mul; // 乘法器流水线暂停信号
assign alu_pipestall = alu_pipestall_div | alu_pipestall_mul;
/*******乘法单元********/
...
assign  mul_inst = (alu_op == `ALU_MUL || alu_op == `ALU_MULH || alu_op == `ALU_MULHSU || alu_op == `ALU_MULHU) ? 1'b1 : 1'b0;

//流水线暂停计数器
always @(posedge clk ) begin
    if (mul_inst == 1'b0 || mul_cnt == MUL_PIPE_STALL_CYCLES) begin
        mul_cnt <= 4'd0;
    end
    else if (mul_inst == 1'b1) begin
        mul_cnt <= mul_cnt + 1'd1;
    end
    else begin
        mul_cnt <= 4'd0;
    end
end
assign alu_pipestall_mul = ((mul_cnt != MUL_PIPE_STALL_CYCLES) && (mul_inst == 1'b1))? 1'b1 : 1'b0;
...
//alu_pipestall_div:除法器流水线暂停信号
always @(*) begin
    if(div_res_rdy == 1'b1) begin
        alu_pipestall_div = 1'b0;
    end
    else if(div_inst == 1'b1) begin
        alu_pipestall_div = 1'b1;
    end
    else begin
        alu_pipestall_div = 1'b0;
    end
